...
def loadFeaturesFromCSV(csvFileName):
    ...
    origfeatureData = genfromtxt(csvFileName, delimiter=',');
    (rows,columns)= origfeatureData.shape
    #print ("rows"+str(columns))
    featureData=numpy.zeros((rows,3))
    #print featureData.shape
    featureData[:,0]=numpy.copy(origfeatureData[:,3])
    featureData[:,1]=origfeatureData[:,4]
    featureData[:,2]=origfeatureData[:,5]
    #print featureData.shape
    return featureData

def loadFeaturesFrom2CSV(csvFileName,inferenceCsvFileName):
    #featureData = csv.reader(open(csvFileName, 'r'));
...
def computeViterbiPath(featureVector,filename):
    ...
    #opArr = [viterbiPath[i,0] for i in range(len(viterbiPath))]

    myfile = open("p"+filename, 'wb')
    wr = csv.writer(myfile, quoting=csv.QUOTE_MINIMAL)
    for i in range(len(viterbiPath)):
        wr.writerow(viterbiPath[i])
...
if __name__ == "__main__":
    ...
    for infile in listing:
        if(infile.endswith('.csv') and infile.startswith("o")):
            ...
            featureData =  loadFeaturesFromCSV(infile)       
            #compute the gaussian observation probabilities
            computeViterbiPath(featureData,"p"+infile)
        #endif
    #endfor
#endif
